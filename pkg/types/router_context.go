...
func (alg RoutingAlgorithms) NewContext(ctx context.Context, model string, message string) *RoutingContext {
	request := requestPool.Get().(*RoutingContext)
	request.reset(ctx, alg, model, message)
	return request
}

func NewRoutingContext(ctx context.Context, algorithms RoutingAlgorithms, model string, message string) *RoutingContext {
	request := requestPool.Get().(*RoutingContext)
...
func (r *RoutingContext) reset(ctx context.Context, algorithms RoutingAlgorithms, model string, message string) {
	r.Context = ctx
